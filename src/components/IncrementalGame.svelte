...
<script>
  ...
  function updateGeneratorCost(generatorName) {
    ...
    switch (generatorName) {
      case "gen1":
        gen1Cost = Math.ceil(
          gen1StartCost * Math.pow(valueMultiplier, gen1Owned)
        );
        break;

      case "gen2":
        gen2Cost = Math.ceil(
          gen2StartCost * Math.pow(valueMultiplier, gen2Owned)
        );
        break;

      case "gen3":
        gen3Cost = Math.ceil(
          gen3StartCost * Math.pow(valueMultiplier, gen3Owned)
        );
        break;

      case "gen4":
        gen4Cost = Math.ceil(
          gen4StartCost * Math.pow(valueMultiplier, gen4Owned)
        );
        break;

      case "gen5":
        gen5Cost = Math.ceil(
          gen5StartCost * Math.pow(valueMultiplier, gen5Owned)
        );
        break;

      case "gen6":
        gen6Cost = Math.ceil(
          gen6StartCost * Math.pow(valueMultiplier, gen6Owned)
        );
        break;
  ...
  function updateIntervalDecrementCost() {
    ...
    intervalDecrementCost = Math.ceil(
      intervalDecrementStartCost * Math.pow(valueMultiplier, numIntervalChanges)
    );
  }
</script>
...
<div class="info">
  ...
  {#if gen1Owned > 0}
    <p>
      automated work per tick: {1 * gen1Owned +
        2 * gen2Owned +
        4 * gen3Owned +
        8 * gen4Owned +
        16 * gen5Owned +
...
<div class="globals">
  <button on:click={incrementCurrency}
    >{#if gen6Owned == 0}
      do work
    {:else}
      do work? why bother? you've been replaced.
    {/if}</button
  >
  {#if currency >= intervalDecrementStartCost && updateInterval > 500}
    <button on:click={decrementInterval}>
      automate work faster (cost: {intervalDecrementCost})
    </button>
  {/if}
</div>
...
<div class="genContainer">
  {#if currency >= gen1StartCost || gen1Owned >= 1}
    <div class="gen">
      ...
      <p>cost: {gen1Cost}</p>
      {#if currency >= gen1Cost}
        <button on:click={() => incrementGenerator("gen1")}>
          buy steam engine
        </button>
      {/if}
    </div>
  ...
  {#if gen1Cost >= gen2StartCost}
    <div class="gen">
      ...
      <p>cost: {gen2Cost}</p>
      {#if currency >= gen2Cost}
        <button on:click={() => incrementGenerator("gen2")}>
          buy assembly line
        </button>
      {/if}
    </div>
  ...
  {#if gen2Cost >= gen3StartCost}
    <div class="gen">
      ...
      <p>cost: {gen3Cost}</p>
      {#if currency >= gen3Cost}
        <button on:click={() => incrementGenerator("gen3")}>
          buy computer
        </button>
      {/if}
    </div>
  ...
  {#if gen3Cost >= gen4StartCost}
    <div class="gen">
      ...
      <p>cost: {gen4Cost}</p>
      {#if currency >= gen4Cost}
        <button on:click={() => incrementGenerator("gen4")}>
          buy ml model
        </button>
      {/if}
    </div>
  ...
  {#if gen4Cost >= gen5StartCost}
    <div class="gen">
      ...
      <p>cost: {gen5Cost}</p>
      {#if currency >= gen5Cost}
        <button on:click={() => incrementGenerator("gen5")}>
          buy ll model
        </button>
      {/if}
    </div>
  ...
  {#if gen5Cost >= gen6StartCost}
    <div class="gen">
      ...
      <p>cost: {gen6Cost}</p>
      {#if currency >= gen6Cost}
        <button on:click={() => incrementGenerator("gen6")}>
          buy digital overlord
        </button>
      {/if}
    </div>
  {/if}
